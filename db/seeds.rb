...
Province.find_each do |province|
  if province_data.key?(province.name)
    ...
    Rails.logger.debug do
      "Updated #{province.name} with GST: #{tax_rates[:gst]}, PST: #{tax_rates[:pst]}, HST: #{tax_rates[:hst]}"
    end
  else
    Rails.logger.debug { "No tax data found for #{province.name}, skipping update." }
  end
end
