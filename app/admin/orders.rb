ActiveAdmin.register Order do
  ...
  form do |f|
    f.inputs "Order Details" do
      f.input :status, as: :select, collection: [ "unpaid", "paid", "shipped", "completed", "canceled" ]
    end
    f.actions
  end
  ...
  show do
    ...
    panel "Order Summary" do
      attributes_table_for order do
        ...
        row("Status") { |order| order.status }
        row("Paypal") { |order| order.paypal_order_id }
      end
    end
  end

  filter :status, as: :select, collection: [ "pending", "paid", "shipped", "completed", "canceled" ]
  filter :created_at
  filter :total_price
end
